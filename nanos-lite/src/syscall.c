...
void do_syscall(Context *c) {
  ...
  switch (a[0]) {
    ...
    case SYS_exit: c->GPRx=0;/*printf("do_syscall(0)\tSYS_exit\t返回值c->GPRx=%d\n",c->GPRx);*/ 
                  ...
                  system_execve("/bin/menu",NULL,NULL);break;//有了开机菜单程序之后, 就可以很容易地实现一个有点样子的批处理系统了. 你只需要修改SYS_exit的实现, 让它调用SYS_execve来再次运行/bin/menu, 而不是直接调用halt()来结束整个系统的运行. 这样以后, 在一个用户程序结束的时候, 操作系统就会自动再次运行开机菜单程序, 让用户选择一个新的程序来运行.
