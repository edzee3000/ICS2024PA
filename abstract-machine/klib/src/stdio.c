...
int vsnprintf(char *out, size_t n, const char *fmt, va_list ap) {
  ...
  while(p[i]!='\0' && j<n)
  {
    if(p[i]=='%')
    {
      ...
      else if( p[i]<='9'&&p[i]>='0'  && p[i+1]=='2'  &&p[i+2]=='d' ){i+=1;  int num=va_arg(ap,int);char tem[10];int k=0;if(num==0){ out[i]='0';j++; if(j>=n)break;}   while(num!=0){int t=num%10;num/=10;tem[k]='0'+t; k++;} tem[k]='\0';for(int l=k-1;l>=0;l--){out[j]=tem[l]; j++;       if(j>=n)break; }  }
