...
//INSTPAT(模式字符串, 指令名称, 指令类型, 指令执行操作);
  ...
  //算术运算指令#######################################################
  INSTPAT("0100000 ????? ????? 000 ????? 01100 11", sub   ,R,{R(rd)=(uint32_t)src1-(uint32_t)src2;});
  INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add   ,R,{R(rd)=(uint32_t)src1+(uint32_t)src2;});//靠一开始这里还写错了，add指令copy过来的时候忘了把-变成+了啊啊啊啊啊
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul   ,R,{R(rd)=(uint32_t)src1*(uint32_t)src2;});
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh  ,R,{R(rd)=(int32_t)(((int64_t)src1*(int64_t)src2)>>32);});//############可能会出问题##############
