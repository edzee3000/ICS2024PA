...
//INSTPAT(模式字符串, 指令名称, 指令类型, 指令执行操作);
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, {R(rd)=s->snpc; /*printf("pc的偏移量为%#x\n",imm*2);*/s->dnpc=(s->pc)+imm*2;
  IFDEF(CONFIG_FTRACE, if(rd==1) trace_func_call(s->pc,s->dnpc););  });//注意在inst_fetch当中已经实现了pc+4的过程，pc没有发生改变，但是snpc和dnpc都已经变化了     
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, {R(rd)=s->snpc; s->dnpc=(src1+imm)&(~(int32_t)1);   //(-1)<<1
  IFDEF(CONFIG_FTRACE, if(rd==1)trace_func_call(s->pc,s->dnpc); 
  else if(rd == 0 && rs1==1) trace_func_ret(s->pc,s->dnpc); /*else printf("有问题，此处pc为:%#x\n",s->pc);*/ );});
  ...
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt   ,B,{if((int32_t)src1<(int32_t)src2) s->dnpc=(s->pc)+imm*2;});
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge   ,B,{if((int32_t)src1>=(int32_t)src2) s->dnpc=(s->pc)+imm*2;});
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh  ,R,{int64_t src3=src1;src3<<=32;src3>>=32; int64_t src4=src2; src4<<=32;src4>>=32;R(rd)=(int32_t)(src3*src4>>32);});//############可能会出问题##############
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu ,R,{uint64_t src3=src1;src3<<=32;src3>>=32; uint64_t src4=src2; src4<<=32;src4>>=32;R(rd)=(uint32_t)(src3*src4>>32);});//############可能会出问题##############
  ...
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret  ,N, {s->dnpc=cpu.CSRs.mepc+4;});//机器模式异常返回    最后执行"异常返回指令"返回到程序触发异常之前的状态.
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti  ,I,{if((int)src1<(int32_t)imm) R(rd)=1;else R(rd)=0;});//有符号数比较
