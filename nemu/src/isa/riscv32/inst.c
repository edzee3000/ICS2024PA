...
//INSTPAT(模式字符串, 指令名称, 指令类型, 指令执行操作);
  ...
  INSTPAT("??????? ????? ????? ??? ????? 01101 11", lui    , U, R(rd)=imm);//########################################lui和li命令待定  这里好像那个有点问题
  ...
  //INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh  ,R,{R(rd)=((int)src1*(int)src2) >> 32;});
  // INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh  ,R,{long long int res=(int)src1*(int)src2; R(rd)=(int)(res>>32);});//############可能会出问题##############
