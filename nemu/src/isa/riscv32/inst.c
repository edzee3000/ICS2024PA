...
//INSTPAT(模式字符串, 指令名称, 指令类型, 指令执行操作);
  ...
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh   ,I,{R(rd)=Mr(src1+imm,2);});//这里可能会有问题！！！！留着待会思考############################################################################
  ...
  INSTPAT("0000000 ????? ????? 000 ????? 0110011", add   ,R,{R(rd)=src1+src2;});//靠一开始这里还写错了，add指令copy过来的时候忘了把-变成+了啊啊啊啊啊
  ...
  //移位操作
  INSTPAT("0000000 ????? ????? 101 ????? 0110011", srl   ,R,{R(rd)=src1>>src2;});//逻辑右移指令
  INSTPAT("0100000 ????? ????? 101 ????? 0110011", sra   ,R,{R(rd)=SEXT(src1,sizeof(src1)*8)>>src2;});//算术右移指令
  INSTPAT("0000000 ????? ????? 001 ????? 0110011", srl   ,R,{R(rd)=src1<<src2;});//逻辑左移指令



  //一些奇奇怪怪的置位指令
  INSTPAT("??????? ????? ????? 011 ????? 0010011", seqz  ,I,{if(src1==0)R(rd)=1;else R(rd)=0;});
  INSTPAT("0000000 ????? ????? 011 ????? 0110011", sltu  ,R,{if((uint32_t)src1<(uint32_t)src2)R(rd)=1;else R(rd)=0;});//无符号数比较
  INSTPAT("0000000 ????? ????? 010 ????? 0110011", slt  ,R,{if((int)src1<(int)src2)R(rd)=1;else R(rd)=0;});//有符号数比较
