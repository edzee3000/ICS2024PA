...
//INSTPAT(模式字符串, 指令名称, 指令类型, 指令执行操作);
  ...
  INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi  ,I,{R(rd)=src1& imm;});//可以用于零扩展



  //移位操作
  INSTPAT("0000000 ????? ????? 101 ????? 01100 11", srl   ,R,{R(rd)=src1>>src2;});//逻辑右移指令
  INSTPAT("0000000 ????? ????? 001 ????? 01100 11", sll   ,R,{R(rd)=src1<<src2;});//逻辑左移指令
  //INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra   ,R,{if((src1>>31)==0)R(rd)>>(src2%32);else {for(int i=0;i<(src2%32);i++)R(rd)=SEXT((src1>>1),31); }  });//算术右移指令   但是这里会不会出现问题？？？！！！！
  //把寄存器 x[rs1]右移 x[rs2]位，空位用 x[rs1]的最高位填充，结果写入 x[rd]。x[rs2]的低 5 位（如果是 RV64I 则是低 6 位）为移动位数，高位则被忽略。
  //为什么要写这么麻烦呢？？因为SEXT进行补位的时候要求len必须是个常数而不能是个变量
  INSTPAT("0000000 ????? ????? 001 ????? 01100 11", slli  ,I,{R(rd)=src1<<imm;});//立即数逻辑左移指令
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli  ,I,{R(rd)=src1>>imm;});//立即数逻辑右移指令
  //把寄存器x[rs1]右移shamt位，空出的位置填入0，结果写入x[rd]。对于RV32I，仅当shamt[5]=0时，指令才是有效的
  //INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai  ,I,{if((src1>>31)==0)R(rd)>>(imm%32);else {for(int i=0;i<(imm%32);i++)R(rd)=SEXT((imm>>1),31); }  });//立即数算术右移指令
